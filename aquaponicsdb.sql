...
create table if not exists systems (id integer primary key not null auto_increment, user_id integer not null references users, name varchar(100) not null, system_id varchar(40) not null, creation_time timestamp not null default now(), start_date date, status integer not null default 0, aqx_technique_id integer references aqx_techniques);
...
create table if not exists system_gb_media (system_id integer not null references systems, gb_media_id integer not null references growbed_media, num integer not null default 0);
